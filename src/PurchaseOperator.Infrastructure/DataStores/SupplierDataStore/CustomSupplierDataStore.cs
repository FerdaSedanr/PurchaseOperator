...
[OrderQuantity] = ISNULL((SELECT SUM(AMOUNT - SHIPPEDAMOUNT) FROM LG_{FirmNumber.ToString().PadLeft(3, '0')}_{PeriodNumber.ToString().PadLeft(2, '0')}_ORFLINE AS ORFLINE WITH(NOLOCK) WHERE ORFLINE.CLIENTREF = SUPPLIER.LOGICALREF),0),
